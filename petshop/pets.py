...
def search(field, value):
    ...
    cursor = conn.cursor()
    cat="cat"
    cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tags_pets t where p.species = s.id and t.pet=p.id and t.tag=(SELECT id FROM tag WHERE name=? )",[value])
